...
static void __init cpu_set_bug_bits(struct cpuinfo_x86 *c)
{
	...
	/* None of the affected CPU has BHI_CTRL */
	if (boot_cpu_data.x86_vendor == X86_VENDOR_INTEL &&
	    !(ia32_cap & ARCH_CAP_ITS_NO) &&
	    !boot_cpu_has(X86_FEATURE_BHI_CTRL)) {
		/*
		 * Guests can't rely on Family/Model or eIBRS, if the VMM did
		 * not expose ITS_NO set the bug unconditionally.
		 *
		 * For host, apart from Family/Model also check for eIBRS support
		 * because SKYLAKE_X and KABYLAKE includes CPUs with and without
		 * eIBRS support, and only the parts with eIBRS are affected.
		 */
		if (boot_cpu_has(X86_FEATURE_HYPERVISOR))
			setup_force_cpu_bug(X86_BUG_ITS);
		else if (cpu_matches(cpu_vuln_blacklist, ITS) &&
			 (ia32_cap & ARCH_CAP_IBRS_ALL))
			setup_force_cpu_bug(X86_BUG_ITS);
	}
