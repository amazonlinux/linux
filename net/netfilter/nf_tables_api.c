					...
					struct nft_hook **first, int *n)
{
	...
	list_for_each_entry_rcu(hook, hook_list, list) {
		if (nft_dump_basechain_hook_one(skb, hook, first, n))
			return -1;
	}
					      ...
					      struct nft_hook **first, int *n)
{
	...
	list_for_each_entry(trans_hook, trans_hook_list, list) {
		if (nft_dump_basechain_hook_one(skb, trans_hook->hook, first, n))
			return -1;
	}
				   ...
				   const struct list_head *trans_hook_list)
{
	...
	if (nft_base_chain_netdev(family, ops->hooknum)) {
		...
		if (hook_list &&
		    nft_dump_basechain_hook_list(skb, hook_list, &first, &n)) {
			goto nla_put_failure;
